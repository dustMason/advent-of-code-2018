...
input.each do |line|
  if (guard = line.match(/Guard #(\w*) /))
    current_guard = guard[1]
  end

  minute = line.match(/:(.*)\]/)[1].to_i

  if /falls/ =~ line
    current_period_start = minute
  elsif /wakes/ =~ line
    guards[current_guard] ||= { amount: 0, periods: [] }
    guards[current_guard][:amount] += (minute - current_period_start)
    guards[current_guard][:periods] << (current_period_start...minute)
    current_period_start = nil
  end
end
...
sleepiest_guard = guards.to_a.sort_by { |(_, hash)| hash[:amount] }.last[0]

def sleepiest_minute(guard_number, guards)
  minutes = guards[guard_number][:periods].map(&:to_a).reduce([]) { |acc, r| acc.concat(r) }
  counts = minutes.each_with_object(Hash.new(0)) { |min, counts| counts[min] += 1 }
  counts.sort_by { |(_min, count)| count }.last
end
...
ranks = guards.map do |num, _|
  min, amount = sleepiest_minute(num, guards)
  { num => { minute: min, amount: amount } }
end
winner = ranks.sort_by { |hash| hash.values[0][:amount] }.last.to_a.first

puts winner.first.to_i * winner[1][:minute]
