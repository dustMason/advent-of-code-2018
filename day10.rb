regex = /position=\<(?<coord>.*)\> velocity=\<(?<velocity>.*)\>/
...
max_y = max_x = -Float::INFINITY

all_points = input.each_line.map do |line|
  x, y, vx, vy = regex.match(line).captures.map { |s| s.split(', ').map(&:to_i) }.flatten
  min_y = y if y < min_y
  max_y = y if y > max_y
  Point.new(x, y, vx, vy)
end
...
until y_delta_delta >= 0
  ...
  new_y_delta = max_y - min_y
  y_delta_delta = new_y_delta - y_delta
  y_delta = new_y_delta
  seconds += 1
end
...
all_points.each do |point|
  2.times { point.untick! }
  field[point.y] ||= []
  field[point.y][point.x] = '.'
end

field.each do |row|
  puts row.map { |c| c || ' ' }.join if row
end
